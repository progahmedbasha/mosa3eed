               ...
               @method('patch')
					<div class="form-row">
                     ...
                     <select class="form-control" name="medicin_id">
                           ...
                           <option value="{{$medicin->id}}" {{($bill_item->medicin_id==$medicin->id)? 'selected':''}}>{{$medicin->name}}</option>
                           @endforeach
                        </select>
                        @error('medicin_id')
						...
						<div class="form-group col-md-6">
							<label for="inputName">Price</label>
							 <input type="text" class="form-control form-control-solid" placeholder="Price" value="{{$bill_item->price}}" name="price">
                        @error('price')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror   
						</div>
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="inputName">Qty</label>
							 <input type="text" class="form-control form-control-solid" placeholder="Qty" value="{{$bill_item->qty}}" name="qty">
                        @error('qty')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror   
						</div>
