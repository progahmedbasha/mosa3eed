						...
						<div class="col">
							...
							<select id="country-dd" class="form-control" name="organization_id">
								<option value="{{$org_shift->organization_id}}" {{($org_shift->organization_id ==
									$org_shift->organization_id)? 'selected' : '' }}>{{$org_shift->Organization->name}}
								</option>
						...
						<div class="col">
							...
							<select id="country-dd" class="form-control" name="branch_id">
								<option value="{{$org_shift->branch_id}}" {{($org_shift->branch_id ==
									$org_shift->branch_id)? 'selected' : '' }}>{{$org_shift->Branch->name}}</option>
					...
					<div class="form-row">
						<div class="col">
							...
							<input type="text" class="form-control form-control-solid" placeholder="Shift Name En"
								value="{{$org_shift->getTranslation('name', 'en')}}" name="name_en">
							@error('name_en')
							<div class="alert alert-danger">{{ $message }}</div>
							@enderror
						</div>
						<div class="col">
							...
							<input type="text" class="form-control form-control-solid" placeholder="Shift Name Ar"
								value="{{$org_shift->getTranslation('name', 'ar')}}" name="name_ar">
							@error('name_ar')
							<div class="alert alert-danger">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="form-row">
						<div class="col">
							...
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="checkbox" id="inlineCheckbox1" value="{{ $day }}"
									name="days[]" checked>
