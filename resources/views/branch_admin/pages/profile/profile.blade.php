...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
						...
						<div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-lg-2 pb-5 pb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
								<!--begin::Heading-->
								<h1 class="d-flex flex-column text-dark fw-bolder my-0 fs-1">Edit User</h1>
								<!--end::Heading-->
								<!--begin::Breadcrumb-->
								<ul class="breadcrumb breadcrumb-dot fw-bold fs-base my-1">
									...
									<li class="breadcrumb-item text-dark">User Edit</li>
								</ul>
								<!--end::Breadcrumb-->
							</div>
							<!--begin::Card-->
                    ...
                    @method('patch')
								 <div class="row gx-10 mb-5">
														...
														<div class="col-lg-6">
															...
															<!--begin::Input group-->
															<div class="mb-5">
																<input type="text" class="form-control form-control-solid" placeholder="Name" value="{{$user->name}}" name="name">
																@error('name')
															...
															<div class="mb-5">
																<input type="text" class="form-control form-control-solid" placeholder="Email" value="{{$user->email}}" name="email">
																@error('email')
																<div class="alert alert-danger">{{ $message }}</div>
																@enderror	
                                                            </div>
															...
															<div class="mb-5">
																<select  id="inputState" class="form-control" style="padding: 10px;" name="user_type_id">
																	<option value="{{$user->user_type_id}}"  {{($user->user_type_id = $user->user_type_id )? 'selected' : '' }}> {{$user->UserType->type}}</option>
																	<option>----------------------------------------------------------</option>
																	@foreach($user_types as $item)
																	<option value="{{$item->id}}" {{(old('user_type_id')==$item->id)? 'selected':''}} >{{$item->type}}</option>
																	@endforeach
																</select>
																@error('user_type_id')
																<div class="alert alert-danger">{{ $message }}</div>
															...
															<div class="mb-5">
																<select  id="inputState" class="form-control" style="padding: 10px;" name="organization_id">
														...
														<div class="col-lg-6">
															...
															<!--begin::Input group-->
															<div class="mb-5">
																<input type="text" class="form-control form-control-solid" placeholder="Phone" value="{{$user->phone}}" name="phone">
																@error('phone')
																<div class="alert alert-danger">{{ $message }}</div>
																@enderror	
                                                            </div>
															...
															<div class="mb-5">
																<input type="text" class="form-control form-control-solid" placeholder="Password" value="{{old('password')}}" name="password">
																@error('password')
																<div class="alert alert-danger">{{ $message }}</div>
																@enderror	
                                                            </div>
															...
															<div class="mb-5">
																	<select id="state-dd" class="form-control"style="padding: 10px;" name="district_id">
																		<option value="{{$user->district_id}}" {{($user->district_id == $user->district_id)? 'selected' : '' }}> {{$user->District->name}}</option>
																	</select>
																	@error('district_id')
																	<div class="alert alert-danger">{{ $message }}</div>
																	@enderror   												
					...
					{{--div for show photo --}}
						...
						<div class="col-xl-12 mb-30">
								<div class="card card-statistics h-100">
									...
									@if(!empty($user->photo))
									<img src="{{url('/data/admins')}}/{{$user->photo }}" class="w3-round" width="200px" alt="Norway">
									@else
