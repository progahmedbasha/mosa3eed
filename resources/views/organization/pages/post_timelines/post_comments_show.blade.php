            ...
            <div class="card card-statistics h-100">
               <div class="card-body">
                  ...
                  <form method="get" class="form-inline" action="{{url('admin/timeline_posts')}}"
                     ...
                     <a href="{{url('admin/timeline_posts')}}" class="btn btn-light-primary me-3"
                        ...
                        <tr>
                           ...
                           <td>
                              <div class="btn-icon-list">
                                 <form action="{{route('comment_status_changes',$post_comment->id)}}" method="POST">
                                    @csrf
                                    {{-- @method('DELETE') --}}
                                    @if ($post_comment->status =='Active')
                                    <button class="menu-link px-3" style="background: transparent;border: 0; color:red;"
                                       data-kt-customer-table-filter="delete_row" value="hide"
                                       name="status">hide</button>
                                    @else
                                    <a href="#">
                                       <button class="menu-link px-3"
                                          style="background: transparent;border: 0; color:blue;"
                                          data-kt-customer-table-filter="delete_row" value="Active"
                                          name="status">Active</button>
                                    </a>
                                    @endif
                                 </form>
                              </div>
